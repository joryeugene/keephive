...
- **Auto-trust pipeline**: `hive improve trust on/off` toggles `.auto-improve-trusted`.
  When enabled, `hive improve review` auto-applies proposals marked `trusted: true` (skills
  and rules generated by `self-improve`) before the interactive loop. Tasks and edits always
  require manual review. `_auto_apply_trusted()` in `commands/improve.py` handles filtering,
  applying, and logging `[AUTO-APPLIED ...]` to `daemon.log`. `self-improve` marks skill and
  rule proposals `trusted: True`; tasks and edits remain untrusted.

- **SSE tab-focus reconnect**: Dashboard (`hive serve`) reconnects the SSE event stream
  automatically when a browser tab regains focus. Prevents stale panels after the tab has
  been backgrounded.

### Fixes

- Stale doc path `~/.claude/hive/specs/` → `~/.keephive/hive/specs/` in
  `data/guides/kingbee-daemon.md`.
- Stale docstring `~/.claude/hive/wander/` → `~/.keephive/hive/wander/` in `storage.py`.
