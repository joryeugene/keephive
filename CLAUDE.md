...
- `commands/improve.py`: `hive improve [list|review|clear-stale]` (alias: `im`). Review and apply KingBee self-improvement proposals from `.pending-improvements.json`. Four types: skill (add guide), task (add daemon task), rule (queue to `.pending-rules.md`), edit (LLM merge into existing guide via haiku, falls back to `$EDITOR` on `ClaudePipeError`). Dismissed items stored in `.dismissed-improvements.json` with rolling cap of 100. `_skill_sync()` auto-fires after skill/task installs.
